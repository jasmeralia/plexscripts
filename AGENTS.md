...
`plexadm.plex.LOCK_BYPASS_COLLECTIONS` lists collections exempt from this guard: `01: Category: Short Videos`, `01: Category: Vertical Video`, `00C: Unrated`, `00A: NO STUDIO`. These describe a fact about the file itself (duration, orientation, rating, studio presence) rather than a judgment call about the content, so locked videos still get tagged into them like any other video. Only add a collection to this set if it's similarly a format/technical property, not a content descriptor - anything content-related stays subject to the full lock.
